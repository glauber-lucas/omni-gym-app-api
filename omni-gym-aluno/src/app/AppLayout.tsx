import { Activity, ClipboardList, CreditCard, FileText, Home, LogOut, Menu, X } from 'lucide-react';
...
const navigation = [
  ...
  { to: '/treino', label: 'Treino', icon: Activity },
      ...
      <header className="sticky top-0 z-30 border-b border-white/70 bg-white/90 shadow-sm backdrop-blur-xl xl:hidden">
        <div className="flex h-16 items-center justify-between px-4">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="Omni Gym Aluno" className="h-11 w-11 rounded-2xl object-cover shadow-sm" />
            <div>
              <span className="block text-sm font-black leading-tight">Omni Gym</span>
              <span className="block text-xs font-bold text-primary-100">Portal do aluno</span>
            </div>
          </div>
      ...
      >
        <div className="flex h-full flex-col">
          <div className="mb-8 rounded-[1.5rem] border border-primary-20 bg-gradient-to-br from-white to-primary-10 p-4 shadow-card">
            <img src="/logo.png" alt="Omni Gym Aluno" className="mx-auto h-24 w-24 rounded-[1.35rem] object-cover shadow-sm" />
            <div>
              <p className="mt-4 text-center text-xs font-black uppercase tracking-wide text-primary-100">Portal do aluno</p>
          ...
          <div className="mt-auto rounded-[1.5rem] border border-slate-100 bg-slate-50/90 p-4">
            <p className="text-xs font-black uppercase tracking-wide text-slate-400">Sessão ativa</p>
            <p className="mt-1 truncate text-sm font-black">{user.name ?? user.email}</p>
            <p className="mt-1 text-xs font-semibold text-ink-60">Treino, documentos e pagamentos em um só lugar.</p>
